...
8. Push your generic image (the one with the root user) to some registry if not done already.
   This will be handy for you, for sharing it with your teammates, and when you open-source your project later.
    ...
    3. Add this link to the TODO ADD PULL_IMAGE_NAME in
       ...
       e.g., ic-registry.epfl.ch/your-lab/your-gaspar/gen_audiowave_.)
...
```bash
git clone <HTTPS/SSH> gen_audiowave
cd gen_audiowave
```
...
./template.sh run python -m gen_audiowave.template_experiment some_arg=some_value
...
```bash
mv gen_audiowave gen_audiowave-tmp
mkdir gen_audiowave
mv gen_audiowave-tmp gen_audiowave/dev
...
E.g., you can mount it at `/project/gen_audiowave` and specify `PROJECT_ROOT_AT=/project/gen_audiowave`.
...
export PROJECT_ROOT_AT=/project/gen_audiowave
apptainer run \
    -c \
    ...
    --env WANDB_API_KEY="" \
    --nv gen_audiowave_amd64-cuda-root-latest.sif
